...
The following is an example settings.xml file which will read required fields from the environment. Don't forget to set environment values (or literal values in the file) for the required fields: `SONATYPE_USERNAME`, `SONATYPE_PASSWORD`, `GPG_KEYNAME`, `GPG_PASSPHRASE`. Using the format `${env.SONATYPE_USERNAME}` allows the usage of environment variables but literal values can be supplied directly where desired.
...
We also publish releases on Github so the created tag should be pushed back to 
the github repo in order to attach release artifacts for direct download.

9. Perform the actual release:
